...
# ╔═╡ 328b59f2-a89f-11ed-36da-9980cfafa6e8
begin
	using Plots
	using PlutoUI
end

# ╔═╡ eef61a67-5639-4555-9ac3-d7fb7d19169a
md""" 
...
# ╔═╡ ae999285-8d21-45d9-8eb6-fec08f4d6aff
md"""
# Vibrating square membrane with its edges fixed
"""
...
begin
	...
	Φ = zeros(length(x),length(y));  # mode shape initialised to zero
	
	function mode_shape(x::AbstractArray, y::AbstractArray, m::Int, n::Int)
		for i ∈ 1:length(x), j ∈ 1:length(y)
			Φ[i,j] = sin(m*π*x[i]/L)*sin(n*π*x[j]/L);
		end
		return Φ
	end
...
# ╔═╡ 0a054471-d5f9-40cc-ba10-f975b54b4959
@bind m PlutoUI.Slider(0:10, show_value=true)

# ╔═╡ 284221b1-d249-4827-b8a3-5c5aa198d6c2
@bind n PlutoUI.Slider(0:10, show_value=true)

# ╔═╡ 0e220eb4-f89a-4a90-b1ee-3eee175d80b5
ϕ = mode_shape(x,y,m,n);

# ╔═╡ 5628161d-55f9-49d4-98b8-bd3552ae58c5
Plots.surface(x,y,ϕ)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "bc394d589fa41435419a051f9fda3eb28c4518ec"
